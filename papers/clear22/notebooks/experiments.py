...
exp_folder = Path(prj_path, "papers/clear22/")
...
def run(model, datasize = 1000, executions=40, logfile=None, output = None, datafile=None, seed = 0, timeout = 1800, simpleOutput = False):
    logfile = logfile or Path(res_folder, f"{strtime()}_log.txt")
    output = output or res_folder
    output.mkdir(parents=True, exist_ok=True)
    #print(model)
    modelfile = Path(model_folder, model)
    params = f"--executions {executions} --datasize {datasize} --output {output} "\
