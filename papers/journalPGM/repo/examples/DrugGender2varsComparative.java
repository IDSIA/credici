...
public class DrugGender2varsComparative {
    ...
    private static void run(int seed) throws IOException, CsvException, InterruptedException, ExecutionControl.NotImplementedException {
        ...
        DataUtil.getJointProb(dataObs, model.getDomain(X,Y));

        TianBounds(model, X, Y);
    ...
    private static void TianBounds(StructuralCausalModel model, int X, int Y) throws InterruptedException, ExecutionControl.NotImplementedException {
        BayesianFactor pXY = (BayesianFactor) DataUtil.getJointProb(dataObs, model.getDomain(X, Y));//BayesianFactor.combineAll(empiricalDist.values()).marginalize(Z);
        double TPubound = pXY.filter(X,0).filter(Y,0).getData()[0] + pXY.filter(X,1).filter(Y,1).getData()[0];
        ...
        double pns_u = res_obs.getDataUpper()[0][0];
        double pns_l = res_obs.getDataLower()[0][0];
        System.out.println("PNS=[" + pns_l + "," + pns_u + "]\t TianAndPearl exogen.");
...
Removing u such that fS(u,T) = 1
PNS=[0.4295,0.4295]	 Datasets: Observational

 */


        //BayesianFactor fy = EquationBuilder.of(m_reduced).fromVector(S, 0,0, 1,0, 1,1);
/*
