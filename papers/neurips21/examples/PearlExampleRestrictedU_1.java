...
public class PearlExampleRestrictedU_1 {
	public static void main(String[] args) throws InterruptedException, ExecutionControl.NotImplementedException {
		...
		// Add new eq fx
		BayesianFactor fx = EquationBuilder.of(smodel).fromVector(X, 0,1,1,1);
		...
		// Add new eq fz
		BayesianFactor fy = EquationBuilder.of(smodel).fromVector(Y, 0,1,0,1,0,1,1,0);
		...
		CausalVE inf_i = inf2.getInferenceList().get(0);
		...
		System.out.println("pns_i= "+inf_i.probNecessityAndSufficiency(X,Y));
		...
		m_i.getFactor(Ux).setData(new double[]{0.7, 0.3});
		m_i.getFactor(Uy).setData(new double[]{0.3, 0.7});
		m_i.getFactor(Uz).setData(new double[]{0.3, 0.7});
		...
		System.out.println(ve.probNecessityAndSufficiency(X,Y));
