...
public class AceInference {
    ...
    public File setNetwork(BayesianNetwork network) throws IOException {
        

        networkFile = File.createTempFile("CrediciAceModel", ".uai");
        
        try(PrintWriter out = new PrintWriter(new FileWriter(networkFile))) {
    ...
    private IndexIterator getIterator(Strides source, Strides target) {
