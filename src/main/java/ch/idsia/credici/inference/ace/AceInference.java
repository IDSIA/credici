...
public class AceInference {
    ...
    private int[] optimizedVariables;
    ...
    public File init(BayesianNetwork network, int[] optimized_variables, boolean table) throws IOException {
        ...
        this.model = network;
        this.optimizedVariables = optimized_variables;

        try { 
            ace = new AceEvalExt(networkFile.getAbsolutePath(), optimizedVariables, acePath, table);
    ...
    public File init(StructuralCausalModel network, boolean table) throws IOException {
    	return init(network.toBnet(), network.getExogenousVars(), table);
//        networkFile = File.createTempFile("CrediciAceModel", ".net");
//        networkFile.deleteOnExit();
//        
//        //Logger.getGlobal().info(networkFile.toString());
...
//        return networkFile;
    ...
    public void update(TIntObjectMap<BayesianFactor> newData) {
        for (int ex : optimizedVariables) {
            BayesianFactor f = newData.get(ex);
            double[] dta = f.getData();
            ace.update_CPTs(ex, dta);
        }    
        this.dirty = true;
    ...
    public Map<Integer, BayesianFactor> compute(TIntIntMap evidence) throws Exception {
        if (dirty) {
            ...
            var res = ace.evaluate(optimizedVariables, evidence);
