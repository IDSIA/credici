    ...
    void merge(StructuralCausalModel model,  StructuralCausalModel[] do_models) {
        ...
        for(int world=1; world<=do_models.length; world++){
            ...
            for(int x: m.getEndogenousVars()){
               ...
               Assert.assertArrayEquals(cfmodel.getMap().getEquivalentVars(world, m.getEndegenousParents(x)), cfmodel.getEndegenousParents(cfmodel.getMap().getEquivalentVars(world, x)));
